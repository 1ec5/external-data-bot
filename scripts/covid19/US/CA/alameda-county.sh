...
curl 'https://services3.arcgis.com/1iDJcsklY3l3KIjE/arcgis/rest/services/AC_dates2/FeatureServer/0/query?where=1%3D1&outFields=Date,BkLHJ_NewCases,BkLHJ_CumulCases,BkLHJ_NewDeaths,BkLHJ_CumulDeaths,ACLHJ_Cases,ACLHJ_CumulCases,ACLHJ_Deaths,ACLHJ_CumulDeaths,AC_Cases,AC_CumulCases,AC_Deaths,AC_CumulDeaths&orderByFields=Date&f=json' > dates.json
...
jq -s '[.[].features[].attributes | .Date = (.Date / 1000 | strftime("%Y-%m-%d"))] | group_by(.Date) | map(reduce .[] as $item ({}; . + $item) | [.Date, .BkLHJ_NewCases, .BkLHJ_CumulCases, .BkLHJ_NewDeaths, .BkLHJ_CumulDeaths, .ACLHJ_Cases, .ACLHJ_CumulCases, .ACLHJ_Deaths, .ACLHJ_CumulDeaths, .AC_Cases, .AC_CumulCases, .AC_Deaths, .AC_CumulDeaths, .Hospitalized_COVID_19_Positive_])' dates.json hosp.json > data.json
