...
(
    ...
    sleep 2; echo '{"delta":true,"handle":2,"method":"GetHyperCubeContinuousData","params":["/qHyperCubeDef",{"qStart":42000,"qEnd":50000,"qNbrPoints":2000},false],"id":115,"jsonrpc":"2.0"}'
    sleep 4
) | websocat -B 135000 'wss://dashboard.cchealth.org/app/b7d7f869-fb91-4950-9262-0b89473ceed6' | tail -n 1 > dashboard.json
# Convert date from number of days since 1899-12-30 to YYYY-MM-DD
jq '.result.qDataPages[].value[].qMatrix | map({date: ((.[0].qNum - 2) * 24 * 60 * 60 | gmtime | .[0] -= 70 | strftime("%Y-%m-%d")), newCases: .[1].qNum})' dashboard.json > newcases.json

# Fetch the total cases by day from the dashboard over JSON-RPC
(
    sleep 2; echo '{"delta":true,"handle":-1,"method":"OpenDoc","params":["b7d7f869-fb91-4950-9262-0b89473ceed6","","","",false],"id":1,"jsonrpc":"2.0"}'
    sleep 2; echo '{"delta":true,"handle":1,"method":"GetObject","params":["jWjFxe"],"id":5,"jsonrpc":"2.0"}'
    sleep 2; echo '{"delta":true,"handle":2,"method":"GetHyperCubeContinuousData","params":["/qHyperCubeDef",{"qStart":42000,"qEnd":50000,"qNbrPoints":2000},false],"id":115,"jsonrpc":"2.0"}'
    sleep 3
) | websocat -B 135000 'wss://dashboard.cchealth.org/app/b7d7f869-fb91-4950-9262-0b89473ceed6' | tail -n 1 > dashboard.json
# Convert date from MM/DD/YYYY to YYYY-MM-DD
jq '.result.qDataPages[].value[].qMatrix | map({date: ((.[0].qNum - 2) * 24 * 60 * 60 | gmtime | .[0] -= 70 | strftime("%Y-%m-%d")), cases: .[1].qNum})' dashboard.json > totalcases.json
...
jq '[.result.qDataPages[].value[].qMatrix | map({date: ((.[0].qNum - 2) * 24 * 60 * 60 | gmtime | .[0] -= 70 | strftime("%Y-%m-%d")), newDeaths: (.[1].qNum + .[2].qNum)}) | foreach .[] as $row (0; . + $row.newDeaths; . as $x | $row | (.deaths = $x))]' dashboard.json > deaths.json
