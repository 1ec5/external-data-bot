...
curl 'https://data.sccgov.org/resource/tg4j-23y2.json' | jq 'def extrapolate: foreach .[] as $row (0; ($row.date // .); . as $x | $row | .date = (.date // ($x | .[:-4] + "Z" | fromdate | gmtime | .[2] += 1 | mktime | todate))); [extrapolate] | map(.date = (.date | split("T")[0]) | {date: (.date | sub("^2012"; "2021") | sub("^1944-01-18"; "2021-01-18")), deaths: (.cumulative | tonumber)})' > deaths.json
