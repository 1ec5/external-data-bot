...
jq '.features | map(.attributes | select(.Total_Cases // .Total_Recovered_ // .Total_Hospitalized // .Total_Deaths) | .Date = (.Date / 1000 | strftime("%Y-%m-%d")) | [.Date, (.Total_Cases | (strings // numbers) | tonumber) // null, (.Total_Recovered_ | (strings // numbers) | tonumber) // null, (.Total_Hospitalized | (strings // numbers) | tonumber) // null, (.Total_Deaths | (strings // numbers) | tonumber) // null])' table.json > casesbyday.json
