...
jq '.features | map(.attributes | select((map_values(strings) | length) > 0) | .Date = (.Date / 1000 | strftime("%Y-%m-%d")) | [.Date, (.Total_Cases | strings | tonumber) // null, (.Total_Recovered_ | strings | tonumber) // null, (.Total_Hospitalized | strings | tonumber) // null, (.Total_Deaths | strings | tonumber) // null])' table.json > casesbyday.json
