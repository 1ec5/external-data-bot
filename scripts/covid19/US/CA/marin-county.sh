...
jq '.features | map(.attributes | .Date = (.Date / 1000 | strftime("%Y-%m-%d")) | [.Date, .Total_Cases // null, .Total_Recovered_ // null, .Total_Hospitalized // null, .Total_Deaths // null])' table.json > casesbyday.json
