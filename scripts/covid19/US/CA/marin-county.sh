...
# Delete placeholder values for when Socrata began reporting data but only partially, in favor of DataWrapper
jq -s --tab '.[0].data = ((.[0].data | map({date: .[0], cases: .[1], selfReportedCases: .[2], recovered: .[3], hospitalized: .[4], deaths: .[5]})) + (.[1] | map({date: .[0], cases: .[1], recovered: .[2], hospitalized: .[3], deaths: .[4]})) + (.[2] | del(.[0].cases) | del(.[0].hospitalized) | del(.[0].deaths)) | group_by(.date) | map(add | [.date, .cases, .selfReportedCases, .recovered, .hospitalized, .deaths])) | .[0]' commons.json casesbyday.json disposition.json | expand -t4
