...
jq -s --tab '.[0].data = ((.[0].data + .[1] | map({date: .[0], cases: .[1], selfReportedCases: null, recovered: .[2], hospitalized: .[3], deaths: .[4]})) + .[2] | group_by(.date) | map(add | [.date, .cases, .selfReportedCases, .recovered, .hospitalized, .deaths])) | .[0]' commons.json casesbyday.json disposition.json | expand -t4
