...
# SELECT * FROM (SELECT date,area,population,cumulative_total_tests,cumulative_positive_tests,cumulative_cases,cumulative_deaths,RANK() OVER (PARTITION BY area ORDER BY date DESC) area_rank FROM "046cdd2b-31e5-4d34-9ed3-b48cdbc4be7a" WHERE date > '2020-01-01') p WHERE p.area_rank = 1
