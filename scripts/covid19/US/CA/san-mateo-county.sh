...
# Update Commons
jq -s --tab 'def eval_repeats(key): foreach .[] as $row (0; ($row[key] // .); . as $x | $row | (.[key] = (.[key] // $x))); .[0] as $commons | ($commons.data | map({date: .[0], newCases: .[1], cases: .[2], deaths: .[3], hospitalized: .[4], hospitalizedInclSuspected: .[5]})) + .[1] + [.[2]] | group_by(.date) | map(map(with_entries(select(.value != null))) | add | [.date, .newCases, .cases, .deaths, .hospitalized, .hospitalizedInclSuspected]) | [eval_repeats(3)] as $data | $commons | .data = $data' commons.json dates.json today.json | expand -t4
