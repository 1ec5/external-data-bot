...
# Update Commons
jq -s --tab '.[1].data = ([.[0].features[].attributes | (.Date / 1000 | gmtime | .[3] -= 7 | mktime | strftime("%Y-%m-%d")) as $date | [$date, .NewCases, .Active, .Deaths, .Recovered, .Cumulative]] | sort_by(.[0])) | .[1]' table.json commons.json | expand -t4
