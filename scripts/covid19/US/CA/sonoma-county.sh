...
jq -s 'def total(key): foreach .[] as $row (0; . + $row[key]; . as $x | $row | (.["total:" + key] = $x)); (.[0].features + .[1].features + .[2].features + .[3].features + .[4].features + .[5].features + .[6].features + .[7].features) | map(.attributes) | group_by(.DateofDeath) | map(add) | [total("Deaths")] | map(.date = (.DateofDeath / 1000 | gmtime | .[3] -= 7 | mktime | strftime("%Y-%m-%d")) | {date: .date, deaths: .["total:Deaths"]})' table.json table2.json table3.json table4.json table5.json table6.json table7.json table8.json > deaths.json
