...
jq '.rows | map(select(.[1] != "") | {(.[1]): (.[2] | tonumber)}) | add | {date: (now | localtime | strftime("%Y-%m-%d")), hospitalized: .["Hospitalized - Cumulative/Hospitalizado Acumulado"]}' dashboard.json > today.json
