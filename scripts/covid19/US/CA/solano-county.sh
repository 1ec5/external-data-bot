...
# Update Commons
jq -s --tab '.[1].data = (((.[1].data | map({date: .[0], newCases: .[1], active: .[2], recovered: .[3], currentlyHospitalized: .[4], totalHospitalized: .[5], deaths: .[6], totalConfirmedCases: .[7]})) + .[0] | group_by(.date) | map(add)) | map(select((.date < "2020-09-03" or .date > "2020-09-09") and .date != "2021-03-15" and .date != "2021-05-10" and .date != "2021-03-14" and .date != "2021-05-09") | [.date, .newCases, .active, .recovered, .currentlyHospitalized, .totalHospitalized, .deaths, .totalConfirmedCases])) | .[1]' casesbyday.json commons.json | expand -t4
