...
jq '.features | map(.attributes | {date: (.date_collected / 1000 | gmtime | .[3] -= 7 | mktime | strftime("%Y-%m-%d")), newCases: .daily_number, active: .cumulative_running_total_number}) | [foreach .[] as $date (0; . + $date.newCases; . as $cases | $date | .cases = $cases)]' table.json > casesbyday.json
...
# Convert date from number of milliseconds to YYYY-MM-DD
jq '.features | map(.attributes | {date: (.date_ / 1000 | gmtime | .[3] -= 7 | mktime | strftime("%Y-%m-%d")), hospitalized: .number_inpatient_by_day})' table.json > hosp.json
